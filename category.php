...
if (isset($_GET['name'])) {
  $category_perma = $_GET['name'];
  $limit = siteinfo('post_per_page');
  $category_name_query = mysqli_query($conn, "SELECT * FROM categorys WHERE permalink = '$category_perma'");
  if (mysqli_num_rows($category_name_query) == 0) {
    header("location:/index.php");
  }
  if (mysqli_num_rows($category_name_query) == 1) {
    while ($category_name_query_row = mysqli_fetch_assoc($category_name_query)) {
      $catName = $category_name_query_row['name'];
      ...
      $post_view_query = "SELECT * FROM posts WHERE category = '$catName' AND status = 'Active' ORDER BY id desc LIMIT {$offset}, {$limit}";
  ...
  <html lang="en">
      <head>
        <title>
          ...
          while ($siteinfo = mysqli_fetch_assoc($site_info_result)) {
            echo $catName . " - " . siteinfo('site_name');
          }
          ?></title>
        <meta charset="utf-8">
      ...
      <body>
        ...
        <div class="container">
          ...
          <div class="row">
            <div class="col-sm-8">
              <div class="index-post search-match-text">
                Showing posts matching the category for <?php echo "&quot;<strong>" . $catName . "</strong>&quot;"; ?>
              </div>
              <?php
          ...
          ?>
          <?php
          $post_page_query = mysqli_query($conn, "SELECT * FROM posts WHERE category = '$catName' AND status = 'Active'");
          ...
          <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
              ...
              if ($page_number > 1) {
                $prev = $page_number - 1;
              ?>
                <li class="page-item"><a class="page-link" href="<?php echo "/category/" . $_GET['name'] . "/" . $prev; ?>">
              ...
              ?>
                <li class="<?php echo $active; ?>"><a class="page-link" href="<?php echo "/category/" . $_GET['name'] . "/" . $i; ?>"><?php echo $i; ?></a></li>
              <?php
              }
              if ($total_page > $page_number) {
                $next = $page_number + 1;
              ?>
                <li class="page-item"><a class="page-link" href="<?php echo "/category/" . $_GET['name'] . "/" . $next; ?>">
